...
def get_exchange_trade(date:str='latest', base:str='USD', return_currency:str=None):
...
def generate_pretty_table_with_hierarchy(data, filter_year=None, exclude_fields=['PurchasePrice'], exclude_total=['Gewinn/Verlust', 'Order', 'Verkaufsdatum', 'Rabatt (inkl.)', 'Type']):
    ...
    for year, plans in grouped_data.items():
        for plan_type, records in plans.items():
            ...
            field_names = ["Year", "PlanType"] + [key for key in existing_keys if key not in ["Year", "PlanType", *exclude_fields]]

            table.field_names = field_names

            # Add rows for the current PlanType, including Year and PlanType
            for record in records:
                row = [year, plan_type] + [format_value(record.get(field, "")) for field in field_names[2:]]
                table.add_row(row)

            table.add_row(["----" for field in field_names])
            # Add the "Total" row for numeric fields
            total_row = ["Total", ""]  # First two fields are Year and PlanType, we leave them as "Total" and empty
            for field in field_names[2:]:
                ...
                if field not in exclude_total:  # Check if all values are numeric
                    total_row.append(format_value(sum([x if isinstance(x, (int,float)) else 0 for x in column_values])))
                else:
                    total_row.append("")  # Non-numeric fields get an empty value in the total row
...
def parse_data(data:list[dict], exchange:bool=False):
    ...
    for entry in data:
        ...
        purchase_price_per_share = entry.get('AdjustedCostPerShare', 0.0) if not espp else entry.get('PurchasePrice', 0.0)
        sell_price_per_share = (entry.get('AdjustedCostPerShare') + entry.get('GainLossPerShare', 0.0)) if not espp else entry.get('AdjustedCostPerShare', 0.0)
        gainloss_per_share = entry.get('GainLossPerShare', 0.0) if not espp else (sell_price_per_share - purchase_price_per_share)
        total_value = sell_price_per_share * entry.get('SharesSold', 0)
        kapitalertrag_usd = gainloss_per_share * entry.get('SharesSold', 0)
        if exchange:
            exchange_rate = get_exchange_trade(date=str(entry.get('SaleDate').strftime('%Y-%m-%d')), return_currency='EUR')
...
def main():
    ...
    parser.add_argument(
        "input_file", type=str, help="Path to the input CSV file containing the broker report."
    )
    parser.add_argument(
    ...
    data = parse_data(read_csv(input_file=args.input_file), exchange=args.include_exchange)
    new_data = []
    if len(data) > 0:
        for elem in data:
